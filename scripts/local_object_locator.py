...
class LocalObjectLocator:
    ...
    def detect_objects_in_img(self, objects_in_im):
        ...
        object_classes = [object_detected.id for object_detected in objects_in_im.objects_detected]

        world_positions = self.get_cam_coords_from_img_positions(object_positions, object_classes, objects_in_im.im_depth, objects_in_im.im_color, objects_in_im.camera_info_color)

        fields = [PointField('x', 0, PointField.FLOAT32, 1),
                  PointField('y', 4, PointField.FLOAT32, 1),
                  PointField('z', 8, PointField.FLOAT32, 1),
                  PointField('cls', 12, PointField.UINT32, 1),
    ...
    def get_cam_coords_from_img_positions(self, im_pixel_positions, object_classes, im_depth, im_color, camera_info):
        """
        ...
        for depth_coord, image_coord, object_class in zip(depth_coords, image_coords, object_classes):
            try:
                ...
                coords = np.append(
                    coords, [[obj_pos.x, obj_pos.y, obj_pos.z, object_class]], axis=0)
                rospy.logdebug_throttle(
        ...
        if self.visualisation and im_color is not None:
            self.visualise_color_and_depth_image(image_color, image_depth, image_coords, depth_coords)

        return coords

    def visualise_color_and_depth_image(self, image_color, image_depth, image_coords, depth_coords):
        # covert images to open_cv
        try:
                # draw circles
